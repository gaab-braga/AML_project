...
for model_name in model_names:
    ...
    # Criar pipeline simulado
    pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('model', RandomForestClassifier(n_estimators=10, random_state=42))
    ])
    pipeline.fit(X, y)  # Treinar o pipeline
    ...
    eval_results = {
        ...
        'pipeline': pipeline  # Adicionar pipeline
    }

    eval_results_list.append(eval_results)

print('Testando plot_threshold_comparison_all_models_optimized com pipeline...')
...
print(f'Pipelines disponíveis: {["pipeline" in er for er in eval_results_list]}')
