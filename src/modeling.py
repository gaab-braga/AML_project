"""
Modeling Module for AML Detection
...
def build_pipeline(config: Dict[str, Any]) -> Pipeline:
    ...
    Args:
        config: Dict with model type and params
    ...
    if model_type == 'rf':
        ...
        model = lgb.LGBMClassifier(**config.get('params', {}))
    else:
    ...
    logger.info(f"Pipeline built with {model_type}")
    return pipeline

def train_pipeline(X, y, pipeline: Pipeline, cv: int = 5, temporal: bool = False) -> Dict[str, Any]:
    """
